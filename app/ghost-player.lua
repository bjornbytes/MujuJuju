...
function GhostPlayer:update()
	...
	local speed = 100 + (20 * ctx.upgrades.muju.zeal)
	...
	if math.distance(self.x, self.y, ctx.player.x, ctx.player.y) > self.maxDis then
		...
		self.x = math.lerp(self.x, ctx.player.x + math.dx(self.maxDis, angle), 8 * tickRate)
		self.y = math.lerp(self.y, ctx.player.y + math.dy(self.maxDis, angle), 8 * tickRate)
	end
