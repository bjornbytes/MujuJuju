...
function UnitAI:moveTowards(target)
  ...
  self.unit.x = self.unit.x + math.min(self.unit.speed * ls.tickrate, math.abs(targetx - self.unit.x)) * lume.sign(targetx - self.unit.x)
  self.unit.animation:set('walk')
  self.unit.animation.flipped = self.unit.x > targetx
end

function UnitAI:runFrom(target)
  self.unit.x = self.unit.x - self.unit.speed * lume.sign(target.x - self.unit.x) * ls.tickrate
  self.unit.animation:set('walk')
